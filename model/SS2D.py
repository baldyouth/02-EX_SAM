...
class SS2D(nn.Module):
    def __init__(self, SS2D_config):

        super().__init__()
        channels = SS2D_config['channels']
        depth = SS2D_config['depth']
        self.fusion_method = SS2D_config['fusion_method']
        self.use_residual = SS2D_config['use_residual']
        self.diag_mode = SS2D_config['diag_mode']
        ...
        diag_dirs = 0
        if self.diag_mode == 'one':
            diag_dirs = 2  # 对角线 \ 的正向和反向
        elif self.diag_mode == 'both':
